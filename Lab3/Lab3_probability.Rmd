...
**Kobe's typical streak length was 0, or no hits. Kobe's longest streak of baskets was 4, which occurred once.**
...
# step 3: create table to view the count up the number of heads and tails
...
**1) Since I set the set the seed above, will it remain the sample for this sample? Should I set the seed for every sample I run in this Rmd?**
**2) Why should it be replace = TRUE for the shot outcomes sample? Answer - b/c of gamblers fallacy, should be replace = TRUE, independent process**

**Response:**
</br>
**To make a sample of 133 shots that reflects a shooting percentage of 45%, you must add in two arguments to your sample function, making the size of the sample 133 and the probability 45% hit and 55% hit. The code to do this is seen below:**
...
**The typical streak length for this simulated independent shooter with a 45% shooting percentage is zero. The player's longest streak of baskets in 133 shots is six. The data is positively skewed and the length ranges from zero to six. The interquartile range is 1, and the standard deviation is 1.32.**
...
**Kobe Bryant's streak data and the simulated shooter's data are both positively skewed. However, the maximum streak length in Kobe Bryant's data was lower than the simulated shooter, and therefore the mean value of Kobe Bryant's data was lower. Using this comparison, it does not seem that the hot hand model fits Kobe's shooting patterns.**
